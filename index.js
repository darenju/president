...
function givePlayerTurn() {
  ...
  if (player) {
    ...
    player.conn.broadcast.emit('playerTurn', name, getPlayerId(playerPlaying));
  }
}
...
function removeCardsFromPlayer(cards) {
  ...
  if (hand.length === 0) {
    ...
    if (winner === -1 && loser !== playerPlaying) {
      ...
      player.conn.broadcast.emit('someoneWon', player.name);
      winner = playerPlaying;
    } else {
      player.conn.emit('finished');
      player.conn.broadcast.emit('someoneFinished', player.name);
    }
  }
  // s'il ne reste que des 2, le joueur a perdu
  else if (hasOnly2s) {
    player.conn.emit('lost');
    player.conn.broadcast.emit('someoneLost', player.name);
    loser = playerPlaying;
    playersFinished.push(playerPlaying);
    nextPlayer();
...
function endGame() {
  ...
  io.emit('gameFinished', winnerName, loserName);
}
...
io.on('connection', function (socket) {
  ...
  socket.on('newPlayer', function (name) {
    ...
    if (getPlayersLength() < 4) {
      io.emit('playersNeeded', 4 - getPlayersLength());
    }
  ...
  socket.on('coup', function (cards) {
    ...
    if ((lastCoupLength() === 0 || (lastCoupLength() > 0 && lastCoupLength() === cards.length)) && isStrongerThanOrEqual(cards[0], lastCard()) && allTheSameCards) {
      ...
      if (lastCard() && lastCard().slice(0, -1) === cards[0].slice(0, -1) && !skippedLast) {
        ...
        skipped.conn.broadcast.emit('someoneWasSkipped', skipper.name, skipped.name, nowPlaying.name, getPlayerId(playerPlaying));
